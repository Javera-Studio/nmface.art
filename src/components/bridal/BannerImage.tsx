...
export function BannerImage({ src, alt, fadeBottom = false, fadeTop = false, rounded = false, height = "h-[60vh]" }: BannerImageProps) {
  return (
    <div className={`relative w-full ${height} min-h-[320px] overflow-hidden ${rounded ? "mx-auto max-w-7xl px-6 sm:px-10" : ""}`}>
      <div className={`relative h-full w-full overflow-hidden ${rounded ? "rounded-2xl" : ""}`}>
        ...
        />
        {fadeTop && (
          <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-[color:var(--cream)] to-transparent" />
        )}
        {fadeBottom && (
